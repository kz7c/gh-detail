...
/*Consts
--------------------*/
const createHeaders = (useToken = true) => ({
  "User-Agent": "gh-detail",
...
const fetchWithFallback = async (url: string) => {
  const authenticatedResponse = await fetch(url, { headers: createHeaders(true) });
  if (authenticatedResponse.status !== 401 || !token) {
    return authenticatedResponse;
  }

  return fetch(url, { headers: createHeaders(false) });
};

const response = await fetchWithFallback(api);
...
for (const repo of repos) {
  try {
    const langResponse = await fetchWithFallback(repo.languages_url);
